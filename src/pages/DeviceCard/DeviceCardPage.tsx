...
export function DeviceCardPage() {
  ...
  useEffect(() => {
    if (Array.isArray(state.value.settingsDriver)) {
      ...
      setFadeTime(getFadeTimeByIndex(fadeT));
      setFadeRate(getFadeRateByIndex(fadeR));
  ...
  const saveBtnSettings = useCallback(() => {
    ...
    saveDriversWithRetry({
      ...
      dr_settyngs: [
        ...
        poweronLevel,
        ...(typeDriver !== 7
          ? [getFadeTimeIndexByValue(fadeTime), getFadeRateIndexByValue(fadeRate)]
          : []), // Добавляем только если typeDriver не 7
        // currentLevel,
      ],
    });
    setFlagPull(true);
